...
# Model path and class names (Reverted to original 2 classes)
...
def load_model(model_path):
    ...
    model.fc = nn.Linear(n_features, len(CLASS_NAMES))  # Only 2 classes
...
# Sub-classify pneumonia 
def classify_sub_pneumonia(label, image_path):
    if label == "Pneumonia":
        filename = os.path.basename(image_path).lower()

        if "covid" in filename:
            return "COVID"
        elif "opacity" in filename or "lungopacity" in filename:
            return "Lung Opacity"
...
def predict(image_path):
    ...
    sub_label = classify_sub_pneumonia(label, image_path)
    return sub_label, confidence, image_tensor
...
def predict_image():
    ...
    try:
        ...
        predicted_class = 1 if label == "Pneumonia" else 0  # Only 2 classes in original model
